-- 08_migrate_about_to_visual.sql
-- Chuyển đổi nội dung trang About Us sang định dạng JSON cho Visual Editor
...
BEGIN
    ...
    -- Cập nhật hoặc chèn mới bản ghi about-us
    INSERT INTO public.static_pages (slug, title, content, is_active)
    VALUES ('about-us', 'Về chúng tôi', about_content::text, true)
    ON CONFLICT (slug) 
    DO UPDATE SET 
