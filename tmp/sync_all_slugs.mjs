...
const SUPABASE_URL = "https://rfzuevsyegqbdlttmloa.supabase.co";
const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InJmenVldnN5ZWdxYmRsdHRtbG9hIiwicm9sZES6ImFub24iLCJpYXQiOjE3NzUwMzQwNDAsImV4cCI6MjA5MDYxMDA0MH0.3nv_Wirt8oaifiJdSkbG4ZaXMCTyN0ZefOti5dkz6ec";
  ...
  "sections": [
    ...
    },
    {
      ...
      "props": {
        ...
        "p3": "Với thế và lực mới, VVC cam kết đồng hành cùng sự thành công của khách hàng thông qua những giá trị thực thi và tính bền vững vượt thời gian.",
        "image": "/vvc-hero.png",
        "expYears": "20+",
...
async function syncAllSlugs() {
  const slugs = ["about-us", "gioi-thieu", "intro"];
  const url = `${SUPABASE_URL}/rest/v1/static_pages`;
  
  for (const slug of slugs) {
    console.log(`Updating slug: ${slug}...`);
    try {
      // First try to PATCH
      const patchResponse = await fetch(`${url}?slug=eq.${slug}`, {
        method: "PATCH",
        headers: {
        ...
        body: JSON.stringify({
          ...
          title: slug === "gioi-thieu" ? "Giới thiệu" : "Về chúng tôi",
      ...
      if (patchedData.length === 0) {
        console.log(`Slug ${slug} not found. Inserting...`);
        // If not found, INSERT
        await fetch(url, {
          method: "POST",
          headers: {
      ...
      console.log(`Slug ${slug} sync done.`);
    } catch (err) {
      console.error(`Error syncing slug ${slug}:`, err);
    }
  }
}
