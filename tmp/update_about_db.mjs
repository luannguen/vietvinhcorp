...
const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InJmenVldnN5ZWdxYmRsdHRtbG9hIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzUwMzQwNDAsImV4cCI6MjA5MDYxMDA0MH0.3nv_Wirt8oaifiJdSkbG4ZaXMCTyN0ZefOti5dkz6ec";
...
async function updateAboutPage() {
  const url = `${SUPABASE_URL}/rest/v1/static_pages?slug=eq.about-us`;
  ...
  try {
    ...
    if (response.ok) {
      console.log("Successfully updated About Us page content in DB.");
    } else {
      const errText = await response.text();
      console.error("Failed to update DB:", errText);
    }
  } catch (err) {
    console.error("Error connecting to Supabase Rest API:", err);
  }
}
