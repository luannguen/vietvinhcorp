...
const slugs = ['about-us', 'intro', 'about-vvc', 'gioi-thieu'];

async function run() {
  console.log('Starting sync for slugs:', slugs);
  for (const slug of slugs) {
    const { data: existing } = await supabase.from('static_pages').select('id').eq('slug', slug).single();
    
    const pageData = {
      slug,
      title: slug === 'intro' ? 'Giới thiệu chung' : 'Về chúng tôi',
    ...
    if (existing) {
      console.log(`Updating existing slug: ${slug}`);
      const { error } = await supabase.from('static_pages').update(pageData).eq('id', existing.id);
      if (error) console.error(`Error updating ${slug}:`, error);
    } else {
      console.log(`Creating new slug: ${slug}`);
      const { error } = await supabase.from('static_pages').insert(pageData);
      if (error) console.error(`Error creating ${slug}:`, error);
    }
  }
